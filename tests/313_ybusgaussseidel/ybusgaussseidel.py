...
def ybusgaussseidel(grid, S, eps_s=1.0, max_iters=10000, acc_factor=1.0):
...
def ybusgaussseidel_plot_each_iteration(grid, S, eps_s=1.0, max_iters=10000, acc_factor=1.0, plotting=True):
        ...
        while True:
            ...
            if plotting and len(U_iters) == 5:
                ...
                for ax in axes:
                    ...
                    ax.grid(True)
                    ax.set_ylim([397, 400])
                    ax.yaxis.set_major_locator(matplotlib.ticker.FixedLocator([397,398,399,400]))
                    ax.yaxis.set_major_formatter(matplotlib.ticker.FixedFormatter(['397 V','398 V','399 V','400 V']))
                    ax.yaxis.set_tick_params(labelsize=7)
                    ax.xaxis.set_major_locator(matplotlib.ticker.FixedLocator([0,1,2,3]))
                    ax.xaxis.set_major_formatter(matplotlib.ticker.NullFormatter())
                    ax.xaxis.set_tick_params(length=0)
                for ax,U_i,Smis_i in zip(axes, U_iters, Smis_iters):
                    Smis_i_mapped = np.interp(Smis_i, [Smis_min, Smis_max], [1, 0.3])
                    ax.plot(np.abs(U_i), '-o', color='green', linewidth=2, markersize=3, zorder=10, clip_on=False)
                    ax.plot([400,399,398,397], '--o', color='lightgreen', linewidth=1, markersize=2, zorder=5, clip_on=False)
                # ax1.set_ylabel('Abs. Voltage / V')
                axes[-1].set_xlabel('Node no.')
                axes[-1].xaxis.set_major_locator(matplotlib.ticker.FixedLocator([0,1,2,3]))
                axes[-1].xaxis.set_major_formatter(matplotlib.ticker.FixedFormatter([0,1,2,3]))
                plt.gcf().text(0.90, 0.9, '0', verticalalignment='center', horizontalalignment='center')
                plt.gcf().text(0.90, 0.725, '1', verticalalignment='center', horizontalalignment='center')
                plt.gcf().text(0.90, 0.55, '2', verticalalignment='center', horizontalalignment='center')
                plt.gcf().text(0.90, 0.375, '3', verticalalignment='center', horizontalalignment='center')
                plt.gcf().text(0.90, 0.2, '4', verticalalignment='center', horizontalalignment='center')
                plt.gcf().text(0.95, 0.5, '$\leftarrow$ Iterations', rotation=90, verticalalignment='center', horizontalalignment='center')
...
def ybusgaussseidel_plot_timings(grid, S, eps_s=1.0, max_iters=100000, acc_factor=1.0):
    ...
    for n in range(3,100):
        ...
        print(f'{n:2.0f} Nodes: {init_time * 1e6:5.0f} us + {int(solve_time / iters * 1e6):3.0f} us x {iters:5.0f} = {solve_time * 1e3:6.1f} ms')
...
U, iters, runtime = ybusgaussseidel_plot_each_iteration(grid,S, plotting=True)
# U, iters, runtime = ybusgaussseidel_plot_timings(grid,S)
