...
def find_candidates_end(grid, S, U0=400 * 0.9, excluded_node_names=[]):
    ...
    for node in nodes:
        ...
        if node["external_name"] in excluded_node_names:
            continue
...
def find_candidates_transit(grid, excluded_node_names=[]):
...
def reduce_one_node_transit(grid, S, excluded_node_names=[]):
    candidates = find_candidates_transit(grid, excluded_node_names=excluded_node_names)
    if not candidates:
        return grid, S, {}
...
if __name__ == "__main__":
    ...
    with open("european_lv_feeder.json", "r") as f:
        grid = json.load(f)
    S = np.load("europeanLVFeeder.npy").astype(np.complex128)
    # plt.plotgraph(grid)
    ...
    np.save(f"U_org_{n}", U_org)
    np.save(f"U_red_{n}", U_red)
    ...
    # plot_voltage_comparison(U_red, U_org)
    # plt.plotgraph(grid_red, S=S_red, filename=f'european_lv_feeder_red_to_{n}')
    ...
    # nodes_in_path = [
    #     node_id_to_node_name[node_name] for node_name in path_of_largest_impedance
    # ]

    # plotgraph_highlight_path(
    #     grid,
    #     S=S,
    #     nodes_in_path=nodes_in_path,
    #     filename="european_lv_feeder_highlightpath",
    # )
