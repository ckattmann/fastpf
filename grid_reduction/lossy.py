    ...
    while q.qsize():
        ...
        for next_node_name, line_impedance in adjacency_dict[node_name]:
            ...
            if visited[next_node_name]:
                continue
            q.put((next_node_name, impedance_to_node, line_impedance, node_path + [node_name]))
...
def reduce_one_node_end(grid, S, U0=400*0.9, excluded_node_names=[]):
    ...
    candidates = find_candidates_end(grid, S, U0=U0, excluded_node_names=excluded_node_names)
    if not candidates:
        return grid, S, {}
    ...
    S[:,parent_node_id] += S[:,node['id']] + (line['R'] + 1j*line['X']) * np.conj(abs(S[:,node['id']]) / abs(U0)) ** 2
...
def reduce_one_node_transit(grid, S, excluded_node_names=[]):
    candidates = find_candidates_transit(grid, S, excluded_node_names=excluded_node_names)
    if not candidates:
        return grid, S, {}
    ...
    if best_candidate['connected_lines'][0]['source'] == node1_id or best_candidate['connected_lines'][0]['target'] == node1_id:
...
def reduce_to_single_feeder(grid,S):
    ...
    # First, try intermediate reduction, then end reduction
    while True:
        grid, S, node_reduced = reduce_one_node_transit(grid, S, excluded_node_names=path_of_largest_impedance)
        if node_reduced:
            nodes_reduced.append(node_reduced)
            continue
        else:
            grid, S, node_reduced = reduce_one_node_end(grid, S, excluded_node_names=path_of_largest_impedance)
            if not node_reduced:
                break
            nodes_reduced.append(node_reduced)
...
def plot_voltage_comparison(U, U_org, filename=None):
    ...
    ax1.xaxis.set_major_locator(ticker.FixedLocator([t*60 for t in (0,6,12,18,24)]))
    ...
    ax2.xaxis.set_major_locator(ticker.FixedLocator([t*60 for t in (0,6,12,18,24)]))
...
def plotgraph_highlight_path(grid, filename='', S=None, ax=None, verbose=False, nodes_in_path=[], **additional_args):
    import copy
    ...
    node_color = ['darkslategray' if n['id'] in nodes_in_path else 'lightgray' for n in grid['nodes']]
    node_color = ['orangered' if not n['noload'] else nc for n,nc in zip(grid['nodes'], node_color)]
    ...
    nx.draw_networkx(g, ax=ax, pos=positions, node_color=node_color, node_size=node_size, edge_color=edge_colors, width=1.5, with_labels=False, **additional_args)
