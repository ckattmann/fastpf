        ...
        for node_i, node in enumerate(nodes):
            ...
            if S is None:
                ...
                if np.any(np.abs(S[:, node["id"]]) > 0):
                    log(
                        f'Node {node["id"]} has load {np.max(np.abs(S)[:,node["id"]])}, skipping'
                    )
                    continue
            ...
            # Transit Node -> Add up connected lines and delete node:
                ...
                if (
                    "length_m" in connected_lines[0].keys()
                    and "length_m" in connected_lines[1].keys()
                ):
                    new_line["length_m"] = (
                ...
                log(
                    f'Node {node["id"]} has {len(connected_nodes)} connected nodes, skipping'
                )
...
def reduce_intermediate_nodes(grid):
    ...
    while node_deleted:
        node_deleted = False
        for node_i, node in enumerate(
            nodes
        ):  # reversed because generally, "chains" of reducable nodes start at the end
...
def reduce_grid(grid):
    ...
    while nodes_reduced_intermediate or nodes_reduced_end:
        ...
        print(
            f'Reduced {nodes_reduced_end} Nodes by end_reduction -> Nodes: {len(grid["nodes"])}, Lines: {len(grid["edges"])}'
        )
        ...
        print(
            f'Reduced {nodes_reduced_intermediate} Nodes by intermediate_reduction -> Nodes: {len(grid["nodes"])}, Lines: {len(grid["edges"])}'
        )
